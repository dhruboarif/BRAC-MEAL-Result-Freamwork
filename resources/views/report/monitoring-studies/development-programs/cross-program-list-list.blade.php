                        ...
                        <table class="table table-hover">
                            ...
                            @foreach($studyArchives as $key=>$archive)
                                <tr>
                                    <td>{{$key+1}}</td>
                                    <td>{{$archive->name}}</td>
                                    <td>{{$archive->program->name}} ({{$archive->program->type}})</td>
                                    ...
                                    <td class="text-right">
                                        <a href="{{route('report.monitoring-studies.development-programs.cross-program-list').'?arch='.$archive->id}}"><button class="btn btn-sm btn-info text-white">Details</button></a>
