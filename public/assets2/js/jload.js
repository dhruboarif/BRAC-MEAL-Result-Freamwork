...
;(function(w,f){
  ...
  Wload.prototype.hide = function(obj){
    if(obj.time){
      var _this = this
      setTimeout(function(){
        $(_this.opt.el).find('.w_load_ta').remove()
        $(_this.opt.el).removeClass('w_load_ta_parent')
      },obj.time)
    }else{
      $(this.opt.el).find('.w_load_ta').remove()
      $(this.opt.el).removeClass('w_load_ta_parent')
    }
  }
  Wload.prototype.bindEvent = function(){}
  Wload.prototype.creatDom = function(){
    var dom = $(this.opt.el)
    var div = $('<div class="w_load_ta"></div>')
    div.append('<div class="w_load_content"><span class="w_load_icon"></span><span class="w_load_content_text">'+ this.opt.text || '' +'</span></div>')
    dom.addClass('w_load_ta_parent').append(div)
